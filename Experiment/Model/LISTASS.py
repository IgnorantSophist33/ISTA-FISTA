...
class LISTASS(nn.Module):
    def __init__(self, A, max_iteration, Lasso_lambda, p):
        super(LISTASS, self).__init__()
        self.A = A
        self.n = A.size(0)
        self.m = A.size(1)
    ...
    def supportSelection(self, x, layer):
        ...
        # x = torch.where(x < - theta or x > theta, x, x_hat)
        # for i in range(max_k.size(0)):
        #     for j in range(max_k.size(1)):
        #         if(x[i, max_k_idx[i, j]] > theta):
        #             x[i, max_k_idx[i, j]] = x[i, max_k_idx[i, j]] + theta
        #         elif(x[i, max_k_idx[i, j]] < -theta):
        #             x[i, max_k_idx[i, j]] = x[i, max_k_idx[i, j]] - theta
        # x = shrinkage(x)
...
def train(x, y, A, max_iteration, Lasso_lambda, lr, pmax):
        ...
        optimizer = torch.optim.SGD(listass.parameters(), lr=lr, momentum=0.9)
        ...
        for epoch in range(1000):
            ...
            for step in range(steps):
                with torch.autograd.set_detect_anomaly(True):
                    ...
                    x_test_hat = listass(y_test)
                    x_comp_hat = listass(y_comp)
