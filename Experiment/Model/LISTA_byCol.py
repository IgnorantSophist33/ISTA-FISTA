...
class LISTA(nn.Module):
    def __init__(self, A, max_iteration, Lasso_lambda):
        ...
        self.W_y = nn.Parameter(torch.zeros((max_iteration, self.m, self.n)))
    ...
    def weights_initialise(self):
        ...
        y = (1/self.L) * self.A.T
        self.W_x = nn.Parameter(x.unsqueeze(0).repeat(self.max_iteration, 1, 1))
        self.W_y = nn.Parameter(y.unsqueeze(0).repeat(self.max_iteration, 1, 1))
        print(self.W_y.shape, self.W_x.shape)

    def forward(self, y):
        shrinkage = nn.Softshrink(torch.abs(self.theta[0]).item())
        x_hat = shrinkage(torch.matmul(self.W_y[0], y))
        ...
        for i in range(self.max_iteration-1):
            ...
            x_hat = shrinkage(torch.matmul(self.W_y[i+1], y) + torch.matmul(self.W_x[i+1], x_hat))
        return x_hat
    # x_hat [m, batch_size]
...
def NMSEdB(x, x_hat):
    vec_temp1 = x - x_hat
    vec_temp2 = x
    # print(vec_temp2.shape)
    norm1 = torch.pow(torch.norm(vec_temp1, p=2, dim=0, keepdim=True), 2)
    norm2 = torch.pow(torch.norm(vec_temp2, p=2, dim=0, keepdim=True), 2)
    # print(norm1.shape, norm2.shape)
    result = torch.sum(10 * torch.log10(torch.div(norm1, norm2))) / x.size(1)
    # print(result)
    return result
...
def train(x, y, A, max_iteration, Lasso_lambda, lr):
        viz = Visdom()
        batch_size = 32
        n_samples = y.size(1) - batch_size

        steps = n_samples // batch_size

        x_test = x[:, x.size(1) - batch_size:y.size(1)]
        y_test = y[:, y.size(1) - batch_size:y.size(1)]
        x_comp = x[:, 0:batch_size]
        y_comp = y[:, 0:batch_size]
        # print("?", y_test.shape, y_comp.shape)
        ...
        for epoch in range(1000):
            ...
            for step in range(steps):
                ...
                # print(x_test_hat.shape, x_test.shape)
