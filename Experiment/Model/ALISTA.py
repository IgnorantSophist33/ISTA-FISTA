...
class ALISTA(nn.Module):
    def __init__(self, A, max_iteration, Lasso_lambda, W):
    ...
    def forward(self, y):
        ...
        x_hat = shrinkage(self.gamma[0] * torch.matmul(y, self.W) - self.theta[0])
        ...
        for i in range(self.max_iteration-1):
            ...
            x_hat = shrinkage(x_hat + self.gamma[i+1] * torch.matmul(y - temp, self.W) - self.theta[i+1])
        return x_hat
...
def train(x, y, A, max_iteration, Lasso_lambda, lr, W):
        viz = Visdom()
        ...
        Alista = ALISTA(A, max_iteration, Lasso_lambda, W)
        # Alista.weights_initialise()

        criterion = nn.MSELoss()
        optimizer = torch.optim.SGD(Alista.parameters(), lr=lr, momentum=0.9)
        ...
        for epoch in range(1000):
            ...
            for step in range(steps):
                ...
                x_test_hat = Alista(y_test)
                x_comp_hat = Alista(y_comp)
